...
- (void)testStandardShopperEvent {
    BranchEvent *event = [BranchEvent standardEvent:BranchStandardEventShopper];
    ...
    XCTAssertNotNil(eventDictionary);
    XCTAssert([eventDictionary[@"name"] isEqualToString:@"SHOPPER"]);
    
    BranchEventRequest *request = [event buildRequestWithEventDictionary:eventDictionary];
    XCTAssert([request.serverURL.absoluteString containsString:@"branch.io/v2/event/standard"]);
}

- (void)testCustomShopperEvent {
    BranchEvent *event = [BranchEvent customEventWithName:@"SHOPPER"];
